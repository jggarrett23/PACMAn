...
domain.margins.prior <- emmeans(unupdate(cogDomain_model), ~ Domain.3, level=hdi_width)
...
model.marginals <- list('Overall'= overall.margins, 'Exercise Intensity' = inten.margins, 
                        'Exercise Type' = type.margins, 'Cognitive Domain'=domain.margins,
                        'Task Time' = time.margins, 'Outcome Measure' = outcome.margins,
                        'Exercise Duration' = duration.margins 
                        )

saveRDS(model.marginals, 'ef_model_marginals.rds')

model.marginals.priors <- list('Overall'= overall.margins.prior, 'Exercise Intensity' = inten.margins.prior, 
                               'Exercise Type' = type.margins.prior, 'Cognitive Domain'=domain.margins.prior,
                               'Task Time' = time.margins.prior, 'Outcome Measure' = outcome.margins.prior,
                               'Exercise Duration' = duration.margins.prior
                              )

saveRDS(model.marginals.priors, 'ef_model_priors_marginals.rds')
...
saveRDS(model.bfs, 'ef_model_parameter_bfs.rds')

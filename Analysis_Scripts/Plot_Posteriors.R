...
# --- Load Models ---
compute_margins = F
include_secondary_moderators = F
...
if (compute_margins){
  ...
  model.marginals <- readRDS('all_model_marginals.rds')
  ...
  if (include_secondary_moderators){
  year.margins <- model.marginals$Year
  design.margins <- model.marginals$Design
  age.margins <- model.marginals$Age
  pcFem.margins <- model.marginals$pcFem
  bmi.margins <- model.marginals$BMI
  vo2.margins <- model.marginals$VO2
  }
}
...
if (include_secondary_moderators){
year.posteriors <- as.data.frame(as.mcmc(year.margins, sep.chains=F))
colnames(year.posteriors) <- year.margins@levels$Date
year.posteriors <- melt(year.posteriors)
names(year.posteriors) <- c('Year', 'beta')

design.posteriors <- as.data.frame(as.mcmc(design.margins, sep.chains=F))
colnames(design.posteriors) <- design.margins@levels$Exercise.Design
design.posteriors <- melt(design.posteriors)
names(design.posteriors) <- c('Design', 'beta')

age.posteriors <- as.data.frame(as.mcmc(age.margins, sep.chains=F))
colnames(age.posteriors) <- age.margins@levels$Age.Centered
age.posteriors <- melt(age.posteriors)
names(age.posteriors) <- c('Age', 'beta')

pcFem.posteriors <- as.data.frame(as.mcmc(pcFem.margins, sep.chains=F))
colnames(pcFem.posteriors) <- pcFem.margins@levels$Percent.Female
pcFem.posteriors <- melt(pcFem.posteriors)
names(pcFem.posteriors) <- c('Percent Female', 'beta')

bmi.posteriors <- as.data.frame(as.mcmc(bmi.margins, sep.chains=F))
colnames(bmi.posteriors) <- bmi.margins@levels$BMI.Centered
bmi.posteriors <- melt(bmi.posteriors)
names(bmi.posteriors) <- c('BMI', 'beta')

VO2.posteriors <- as.data.frame(as.mcmc(vo2.margins, sep.chains=F))
colnames(VO2.posteriors) <- vo2.margins@levels$VO2.Centered
VO2.posteriors <- melt(VO2.posteriors)
names(VO2.posteriors) <- c('VO2', 'beta')
    
}
#height.posteriors <- as.data.frame(as.mcmc(height.margins, sep.chains=F))
#colnames(height.posteriors) <- height.margins@levels$Height.Centered
#height.posteriors <- melt(height.posteriors)
#names(height.posteriors) <- c('Height', 'beta')

#weight.posteriors <- as.data.frame(as.mcmc(weight.margins, sep.chains=F))
#colnames(weight.posteriors) <- weight.margins@levels$Weight.Centered
#weight.posteriors <- melt(weight.posteriors)
#names(weight.posteriors) <- c('Weight', 'beta')
...
ggsave('Overall_Model_mu_ecdf.jpg', plot=overall_effect.ecdf_plot, path=plotDir,
       units='in', width=2.65, height=1.98)
...
ggsave('Overall_Model_savageD.jpg', plot=savageD_plot, path=plotDir,
...
domain_plot <- ggplot(domain.posteriors, aes(x=beta, y=Domain)) + 
  ...
  scale_y_discrete(expand=c(0,0), 
                   labels=c('Attention', 'Executive Function', 'Information\nProcessing',
...
exType_plot <- ggplot(type.posteriors, aes(x=beta, y=ExType)) + 
  ...
  scale_y_discrete(expand=c(0,0), 
                   labels=c('Circuit', 'Cycling', 'HIIT', 'Resistance', 'Running', 'Sport Activity', 'Walking')) +
  coord_cartesian(clip='off') +
  theme_minimal(base_size=14) +
  theme(
    plot.title = element_text(size=11,hjust = 0.5),
...
dur.labels <- c('\U2264 16','20-27','30-35', '40-45','\U003E 60', 'undefined')
